...
async def main():
    with open('ARIA_Master_Documentation.md', 'r') as f:
        md_text = f.read()

    # We need to account for relative paths in the images, let's fix them to absolute paths for Playwright rendering
    abs_dir = os.path.abspath('.')
    md_text = md_text.replace('(../data/raw/', f'file://{abs_dir}/data/raw/')
    md_text = md_text.replace('(data/raw/', f'file://{abs_dir}/data/raw/')
    ...
    <style>
      ...
      img { max-width: 80%; height: auto; display: block; margin: 20px auto; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    ...
    with open('temp_master.html', 'w') as f:
        f.write(final_html)

    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
        await page.goto(f"file://{os.path.abspath('temp_master.html')}", wait_until="networkidle")
        await page.pdf(
            path='ARIA_Combined.pdf',
            format='A4',
            print_background=True,
            display_header_footer=True,
            header_template='<div></div>',
            footer_template='<div style="font-size:10px; width:100%; text-align:center; color:#9ca3af;"><span class="pageNumber"></span> / <span class="totalPages"></span></div>',
