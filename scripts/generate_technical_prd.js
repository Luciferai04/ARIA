const puppeteer = require('/Users/soumyajitghosh/Documents/codexAxiom/Code Generation Map/codexmap/node_modules/puppeteer');
const fs = require('fs');
const path = require('path');

async function generatePDF() {
  const writeupPath = path.join(__dirname, '..', 'ARIA_Technical_PRD.md');
  const outputPath = path.join(__dirname, '..', 'ARIA_Technical_PRD.pdf');

  if (!fs.existsSync(writeupPath)) {
    console.error('ARIA_Technical_PRD.md not found!');
    process.exit(1);
  }
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>ARIA Technical PRD</title>
  ...
  const previewPath = path.join(__dirname, '..', 'ARIA_Technical_PRD_Preview.png');
  ...
  await browser.close();
  console.log(`[PDF] Success! Technical PRD saved to: ${outputPath}`);
}
